...
function website(v: unknown, errs: Record<string, string>) { const s = opt(v, 2048); if (!s) return undefined; try { const u = new URL(s); if (!["http:", "https:"].includes(u.protocol) || ["localhost", "127.0.0.1", "::1"].includes(u.hostname)) errs.website = "Enter a valid public http or https URL."; } catch { errs.website = "Enter a valid public http or https URL."; } return s; }
