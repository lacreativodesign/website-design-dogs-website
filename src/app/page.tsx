...
export default function Home() { return <>
  ...
  <section className="home-section home-industries" aria-labelledby="home-industries-title"><Container><SectionHeading number="03" eyebrow="INDUSTRIES" id="home-industries-title" title={<>Different Businesses.<br />One Clear <em>Next Step.</em></>} /><p className="home-industries__intro">We shape each website around the way its audience compares, decides, and gets in touch.</p><div className="home-industries-grid">{industries.map((industry) => <Link key={industry.slug} href={`/get-started?industry=${industry.slug}`} className="home-industry-card"><span aria-hidden="true">{industry.marker}</span><h3>{industry.title}</h3><p>{industry.description}</p><strong>{industry.websiteFocus}</strong><b aria-hidden="true">→</b></Link>)}</div></Container></section>
