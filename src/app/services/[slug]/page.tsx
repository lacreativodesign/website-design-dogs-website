...
const serviceStepIcons: BrandIconName[] = ["search", "pen-tool", "code-2", "rocket"];
  ...
  return <><BreadcrumbJsonLd items={[{ name: "Home", path: "/" }, { name: "Services", path: "/services" }, { name: service.title, path: `/services/${service.slug}` }]} /><WebPageJsonLd name={service.title} description={service.valueProposition} path={`/services/${service.slug}`} /><ServiceJsonLd service={service} /><FaqJsonLd items={service.faqs} />
    <section className="service-detail-hero" aria-labelledby="service-title">
      <ThemeScene
        ...
        alt={`Border collie website designer arranging the tools for ${service.title}`}
        width={1600}
        height={1000}
        priority
        sizes="100vw"
      ...
      <Container className="service-detail-hero__layout">
        <div className="service-detail-hero__copy">
          ...
          <div className="service-detail-hero__actions">
            ...
            <Button href="/services" variant="outline">Back to All Services</Button>
          </div>
          <ul className="service-detail-hero__proof" aria-label={`${service.title} priorities`}>
            {["Clear scope", "Responsive execution", "Practical handoff"].map((item) => <li key={item}><BrandIcon name="clipboard-check" />{item}</li>)}
          </ul>
        </div>
      </Container>
    </section>
