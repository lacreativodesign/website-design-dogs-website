...
export default function PortfolioPage() {
  return <>
    ...
    <section className="home-section portfolio-process" aria-labelledby="portfolio-process-title"><Container><PortfolioHeading number="05" eyebrow="YOUR PROJECT" id="portfolio-process-title" title={<>A Design Built<br />Around Your Business.</>} /><div>{portfolioProcess.map(([, title, body], index) => <article key={title}><span className="inner-card-icon"><BrandIcon name={portfolioProcessIcons[index] ?? "clipboard-check"} /></span><h3>{title}</h3><p>{body}</p></article>)}</div></Container></section>
