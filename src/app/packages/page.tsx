...
const packageTrustIcons: BrandIconName[] = ["clipboard-check", "search-check", "smartphone", "heart-handshake"];
const packageValueIcons: BrandIconName[] = ["brush", "gauge", "search-check", "shield-check", "headset"];
...
export default function PackagesPage() {
  return <>
    ...
    <section className="packages-trust-strip" aria-label="Package assurances"><Container><div>{packageTrustItems.map((item, index) => <p key={item}><BrandIcon name={packageTrustIcons[index] ?? "clipboard-check"} />{item}</p>)}</div></Container></section>
    <section className="packages-section" aria-labelledby="package-pricing-title"><Container><SectionHeading number="02" eyebrow="Website Packages" title="A Clear Starting Point for Every Stage." id="package-pricing-title" /><div className="packages-card-grid">{websitePackages.map((pkg) => <article key={pkg.slug} data-package={pkg.slug} className={pkg.label ? "packages-card packages-card--recommended" : "packages-card"}>{pkg.label ? <p className="packages-card__label">{pkg.label}</p> : null}<h3>{pkg.name}</h3><p className="packages-card__price">{pkg.price} <span>One Time</span></p><p className="packages-card__description">{pkg.description}</p><ul>{pkg.features.map((feature) => <li key={feature}>{feature}</li>)}</ul><div className="packages-card__actions"><PackageLeadButton href={pkg.quoteHref} slug={pkg.slug} featured={Boolean(pkg.label)}>Order Now</PackageLeadButton><Button href={pkg.href} variant="outline">View Details</Button></div></article>)}</div><div className="packages-value-rail" aria-label="Included website value">{packageValueItems.map(([title, body], index) => <article key={title} className="value-benefit-tile"><BrandIcon name={packageValueIcons[index] ?? "brush"} /><div><h3>{title}</h3><p>{body}</p></div></article>)}</div></Container></section>
